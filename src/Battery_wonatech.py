...
class LIB_cyc:
    def __init__(self, CYC):
        ...
        self.raw = pd.read_csv(
            ...
            encoding="cp949",
            index_col=0,
            usecols=["Index", "Cycle_No.", "Dischg.Q(Ah)", "Dischg.Q_s(Ah/g)"],
        )
...
class LIB_tot:
    ...
    def get_separation(self, mass):
        ...
        # complete_name = os.path.join(parent_path, "battery.pqt")

        # # print(dummy)
        # self.raw.to_parquet(complete_name)
...
class LIB_sep:
    ...
    def get_GCD(self):
        ...
        if cv_step is None:
            self.df = pd.concat(
                [
                    self.raw[self.raw.step == charge_step][cols].reset_index(drop=True),
                    self.raw[self.raw.step == discharge_step][cols].reset_index(
                        drop=True
                    ),
                ],
                axis=1,
            ...
            charge_with_cv = pd.concat(
                [
                    self.raw[self.raw.step == charge_step][cols],
                    self.raw[self.raw.step == cv_step][cols],
                ]
            )

            discharge = self.raw[self.raw.step == discharge_step][cols].reset_index(
                drop=True
            )
            ...
            self.df = pd.concat(
                [
                    charge_with_floor[cols].reset_index(drop=True),
                    self.raw[self.raw.step == discharge_step][cols].reset_index(
                        drop=True
                    ),
                ],
