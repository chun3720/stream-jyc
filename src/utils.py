...
def supycap(file, example = False):
    ...
    supercap = Load_capacitor(read_file, ESR_method=2, current = curr, cap_grav =  False, cap_method =1)
    fig, ax = plt.subplots()
    fig = supercap.Check_analysis(begin =1, end = len(sc1.cycles)-1)
    st.set_option('deprecation.showPyplotGlobalUse', False)
    st.pyplot(fig)
    
    supercap2 = Load_capacitor(read_file, ESR_method=2, current = curr, cap_grav =  False, cap_method =2)
    ...
    ax2.scatter(range(cap_results.shape[0]), cap_results*1000, s = 20**2, marker = 'o', c = "r", alpha=0.5, label = "method1: lower half")
    ax2.scatter(range(cap_results2.shape[0]), cap_results2*1000, s = 20**2, marker = 'o', c = "b", alpha=0.5, label = "method2: upper half")
